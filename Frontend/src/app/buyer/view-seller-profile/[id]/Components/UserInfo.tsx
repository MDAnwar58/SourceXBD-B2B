...
type Props = {
   sellerInfo: SellerInfo | any;
};

export default function UserInfo({ sellerInfo }: Props) {
   const localUrl: string = LocalUrl();
   let sellerImage: string = "";
   let coverPhoto: string = "";
   if (sellerInfo?.image !== "") {
      const storeImagePath = sellerInfo?.image;
      const forwardSlash = "/";
      const imagePath = forwardSlash.concat(storeImagePath);
      const imageUrl = localUrl.concat(imagePath);
      sellerImage = imageUrl;
   }
   if (sellerInfo?.cover_photo !== "") {
      const imagePath = sellerInfo?.cover_photo;
      const imageUrl = localUrl.concat(imagePath);
      coverPhoto = imageUrl;
   }
