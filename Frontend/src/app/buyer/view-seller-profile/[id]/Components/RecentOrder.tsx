...
export default function RecentOrder({ order, i }: Props) {
   let date: string = "";
   if (order?.created_at) {
      const dateFormat = formatDate(order?.created_at);
      date = dateFormat;
   }
   return (
      <div className="bg-[#f5f5f5] rounded-[13px] flex items-center gap-x-3 mb-3 p-3">
         <div className="xs:hidden block text-[#2f2f2f] text-left font-['Arial-Regular',_sans-serif] text-xs font-semibold">
            {i + 1}
         ...
         <div className="xs:flex items-center justify-between w-full">
            <div className="flex items-center gap-x-3">
               <div className="xs:block hidden text-[#2f2f2f] text-left font-['Arial-Regular',_sans-serif] text-xs font-semibold">
                  {i + 1}
                  {/* <CheckBox className="bg-[#f5f5f5] text-[#515151] border-solid border-[#515151] border focus:ring-[#515151] rounded-sm w-3 h-3 " /> */}
               </div>
