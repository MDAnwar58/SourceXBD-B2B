...
export default function UserAvailableDetails({ userAvailable }: Props) {
   const iso = userAvailable?.iso !== null && userAvailable?.iso.toLowerCase();
         ...
         <div className=" xs4:grid grid-cols-2 gap-x-2 mt-3">
                     ...
                     <div className="w-5 h-5">
                        {userAvailable?.iso ? (
                           <Img
                              ...
                              width={50}
                              height={50}
                              className="h-full w-full rounded-[0.59rem]"
                           />
                        ) : null}
                     </div>
                  </div>
            ...
            <div>
               ...
               <Input
                  ...
                  placeholder=" Daulotpur, Khulna"
                  defaultValue={`${userAvailable?.state}`}
               />
            </div>
         </div>
         ...
         <Button
            ...
            className={` capitalize ${
               userAvailable?.status === "active"
                  ? "bg-[rgba(144,255,56,0.80)]"
                  : "bg-red-500/80"
            } text-[#ffffff] text-left font-['Raleway-Bold',_sans-serif] text-xs font-bold rounded px-7 py-1 xs4:ms-1`}
         >
            {userAvailable?.status}
         </Button>
      </div>
   );
}
