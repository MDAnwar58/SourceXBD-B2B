...
export default function MessageContent({ params }: Props) {
   ...
   useEffect(() => {
      ...
      if (params) {
         setReceiverId(Number(params?.receiveId));
         dispatch(
            getReceiverWithMessage({
               receiverId: Number(params?.receiveId),
               limit: limit,
            })
         );
         socket.emit("sendChatToSourceBDXServer", null);
   ...
   useEffect(() => {
      ...
      socket.on("sendChatToSourceBDXClient", (data) => {
         if (data !== null) {
            ...
            dispatch(
               getRecentMessageUserList({ limit: recentUserLimit, search })
            );
         }
      });
