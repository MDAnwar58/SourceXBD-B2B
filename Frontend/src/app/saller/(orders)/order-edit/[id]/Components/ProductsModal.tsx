...
export default function ProductsModal({
   ...
   const {
      ...
      totalProductsLength,
      loading,
   } = useSelector((state: SallerState) => state.saller.orders);
   const Products: Array<Product[]> = products;

   return (
      <Fragment>
         <div
            className={`${productSelectionModal === false ? "hidden" : ""} fixed overflow-y-auto overflow-x-hidden top-0 right-0 left-0 z-[60] flex justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full`}
         >
         ...
         {productSelectionModal !== false ? (
            <div className="bg-gray-900/50 backdrop-blur-lg dark:bg-gray-900/80 w-full h-full fixed inset-0 z-50"></div>
         ) : null}
      </Fragment>
   );
}
