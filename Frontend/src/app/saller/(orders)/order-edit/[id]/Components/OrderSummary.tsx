...
export default function OrderSummary({
   ...
   const { loading } = useSelector((state: SallerState) => state.saller.orders);
   useEffect(() => {
      ...
      <div>
         ...
         <div className="flex justify-end pt-5">
            <Button
               ...
               className={` 
               ${
                  loading === false ? " text-[#515151]" : "text-[#515151]/30"
               }  bg-[#f0f0f0] capitalize text-left font-['Raleway-Bold',_sans-serif] text-sm font-bold rounded-[14px] px-7 h-[42px] flex justify-center items-center transition-all duration-100 ease-linear`}
               onClick={() => updateOrder(Number(params?.id))}
            >
               Update Order
            </Button>
         </div>
