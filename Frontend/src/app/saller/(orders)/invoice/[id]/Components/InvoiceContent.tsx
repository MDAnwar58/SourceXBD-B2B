...
import { getOrderInvoice } from "@/saller/orders/features/OrdersAction";
import { SallerState } from "@/app/saller/store";
import { useRouter } from "next/navigation";
import { SpinSvgIcon } from "@admin/components/SvgIcons";
import dynamic from "next/dynamic";
const PageHeader = dynamic(() => import("@/app/saller/components/PageHeader"));
...
export default function InvoiceContent({ params }: Props) {
   ...
   const goToPrintInvoicePage = () => {
      setLoading(true);
      setTimeout(() => {
         setLoading(false);
         router.push(`/invoice-print/${Order?.id}`);
      }, 1500);
   };

   return (
      <Fragment>
         <PageHeader icon={icon} title="Invoice" searchBox={false} />
         ...
         <div className="rounded-md lg:col-span-5" ref={invoiceArea}>
            ...
            <div className="rounded-md ">
               ...
               <div className="flex justify-end gap-4 lg:gap-9 mt-5 mb-5">
                  <Button
                     type="button"
                     className={`${loading === true ? " bg-blue-gradient-disable flex-row gap-1" : " bg-blue-gradient"} rounded-[14px] w-[201px] h-[42px] flex justify-center items-center text-[#ffffff] font-['Raleway-Bold',_sans-serif] text-sm font-bold`}
                     onClick={() => goToPrintInvoicePage()}
                  >
                     Print Process
                     {loading === true ? (
